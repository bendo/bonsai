...
void loop() {
    bool readingButtonA = digitalRead(BUTTON_A);

    if (readingButtonA != lastButtonState) {
        delay(DEBOUNCE_TIME); // Wait for debounce period
        readingButtonA = digitalRead(BUTTON_A); // Re-read the button
    }

    if (readingButtonA != buttonState) {
    ...
    if (buttonState == LOW && !isLongPressDetected) {
    ...
    lastButtonState = readingButtonA; // Update last state for next iteration
